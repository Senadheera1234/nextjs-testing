...
export default function Verification() {
    ...
    // Move focus to the next digit box when a single number key is pressed
    const focus = (e: KeyboardEvent<HTMLInputElement>) => {
        const isDigit = /^[0-9]$/.test(e.key);
        if (!isDigit) return;

        // InputNumber renders: <span class="p-inputnumber"><input/></span>
        // so we need the input's parent <span> and then its next sibling <span>
        const nextWrapper = e.currentTarget.parentElement?.nextElementSibling as HTMLElement | null;
        const nextInput = nextWrapper?.querySelector('input') as HTMLInputElement | null;
        nextInput?.focus();
    };
    ...
    return (
        ...
        >
            ...
            <div className="w-full" style={{ background: 'var(--surface-ground)' }}>
                <div
                    className="p-fluid min-h-screen bg-auto md:bg-contain bg-no-repeat text-center w-full flex align-items-center md:align-items-start justify-content-center flex-column"
